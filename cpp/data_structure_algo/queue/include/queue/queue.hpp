...
public:
    ...
    void emplace_back(Args&&... args) {
        if (m_map.empty() || m_map.back()->backSpace() == 0) {
            auto new_back = new buffer_type(m_alloc);
            try {
                new_back->spareBack(BLOCK_LEN);
                new_back->empalceBack(std::forward<Args>(args)...);
                m_map.empalceBack(new_back);
            } catch (...) {
                delete new_back;
                throw;
            }
        } else {
            m_map.back()->empalceBack(std::forward<Args>(args)...);
        }
    }
    ...
    void pop_front() {
        m_map.front()->popFront();
    }

    template<typename ...Args>
    void emplace_front(Args&&... args) {
        if (m_map.empty() || m_map.front()->frontSpace() == 0) {
            auto new_front = new buffer_type(m_alloc);
            try {
                new_front->spareFront(BLOCK_LEN);
                new_front->empalceFront(std::forward<Args>(args)...);
                m_map.empalceFront(new_front);
            } catch (...) {
                delete new_front;
                throw;
            }
        } else {
            m_map.front()->empalceFront(std::forward<Args>(args)...);
        }
    }
