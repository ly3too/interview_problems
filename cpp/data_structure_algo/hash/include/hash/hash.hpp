...
public:
    static uint64_t compute(const void *s, std::size_t len) {
        uint64_t crc = 0;
        auto b = reinterpret_cast<const char *>(s);
        for (int i = 0; i < len; i++) {
...
const std::uint64_t Crc64::Table[256] = {
...
uint64_t DJB2Hash(const void *p, std::size_t len) {
    uint64_t hash = 5381;
    auto pc = reinterpret_cast<const char *>(p);
...
// partial special for enum int
template<typename T, decltype(DJB2Hash) HashFun = Crc64::compute, typename = void>
struct Hash {
...
template<decltype(DJB2Hash) HashFun>
struct Hash<const char*, HashFun> {
    static std::size_t hash(const char* p, std::size_t len) {
        return HashFun(p, len);
    }
};

template<decltype(DJB2Hash) HashFun>
struct Hash<std::string, HashFun> {
...
std::size_t MyHash(const T &val) {
    return Hash<T, HashFun>::hash(val);
}

template<decltype(DJB2Hash) HashFun = Crc64::compute>
